...
#include <unistd.h>
#include <string.h>
#include </home/ahmad/ext2_fs.h>

#define BASE_OFFSET 1024                   /* locates beginning of the super block (first group) */
#define FD_DEVICE "fd0"               /* the floppy disk device */
#define BLOCK_OFFSET(block) (BASE_OFFSET+(block-1)*block_size)

static unsigned int block_size = 0;        /* block size (to be calculated) */

static void read_dir(int, const struct ext2_inode*, const struct ext2_group_desc*, const char*);
...
static void read_file_content(int fd, const struct ext2_inode *inode)
{
    void *block;
    
    //if (S_ISREG(inode->i_mode)) {
        
        block = malloc(block_size * inode->i_blocks);

        for (int i = 0; i < inode->i_blocks; ++i) {
            lseek(fd, BLOCK_OFFSET(inode->i_block[i]), SEEK_SET);
            read(fd, block + i * block_size, block_size);
        }
        printf("File content:\n%s\n", (char *)block);
...
static void read_dir(int fd, const struct ext2_inode *inode, const struct ext2_group_desc *group, const char *target_file)
{
    void *block;
    ...
    if (S_ISDIR(inode->i_mode)) {
        ...
        while ((size < inode->i_size) && entry->inode) {
            ...
            if (strcmp(file_name, target_file) == 0) {
                ...
                read_inode(fd, entry->inode, group, &file_inode);
                read_file_content(fd, &file_inode);
            }
